...
if( isset($_POST['submit']) && isset($_POST['idfb']) && isset($_POST['amount'])){
$amount=$_POST['amount'];
$idfb=$_POST['idfb'];
...
$url="https://muasubngon.com/api/service/order-follow";
...
$ch = curl_init();
    ...
    curl_setopt( $ch, CURLOPT_POSTFIELDS, "idfb=".$idfb."&amount=".$amount."&note=".$note);
	$headers = array(
...
$create = $ketnoi->query("INSERT INTO `sub` SET 
    ...
    <div class="row">
        <div class="col-md-12">
            <div class="card m-b-30">
                <div class="card-header"><h5 class="card-title m-b-0">Thêm đơn</h5></div>
                <div class="card-body">
                    <form action="" method="post">
                                                    
                        <div class="form-group row">
                            <label class="col-md-2">IDFB:</label>
                            <div class="col-md-10"><input type="text" class="form-control" name="idfb" placeholder="Nhập link bài viết cần tăng" value="" /></div>
                        </div>
                        <div class="form-group row">
...
<ul class="pt-2"><li>Mua bằng ID Facebook đã mở chế độ theo dõi công khai, phải hiển thị nút theo dõi</li><li>Tất cả đều là người dùng Việt Nam.</li><li>Đơn càng to thì lên sẽ càng nhanh</li><li>Hiện tại hệ thống có thể tăng tối đa 1m follow / 1 profile</li><li>Sau khi mua thì chỉ từ 1p đến 20p bắt đầu chạy.</li><li>Chế độ bảo hành trong 10 ngày khi tụt trên 10% sẽ được buff lại số đã tụt (không bảo hành với dạng page chuyển sang fb cá nhân).</li><li>Chúng tôi không hoàn tiền cho đơn đã thanh toán.</li></ul>
                                </div>
                            </div>
                        </div>
                        <div class="form-group row">
                            <div class="col-sm-12 text-center">
                                <div class="alert alert-dismissible alert-success">
                                    <h3 class="font-bold">
                                        Tổng thanh toán: <span class="bold green"><span id="total_payment" class="text-danger"><?=$ratesub * 100?></span> VNĐ</span>
                                    </h3>
                                </div>
                            </div>
                        </div>
                                    ...
                                    <table role="table" class="table table-sm table-bordered table-striped table-hover">
                                        <thead>
                                            <tr role="row">
                                                ...
                                                <th colspan="1" role="columnheader">
                                                    <div title="Toggle SortBy" class="text-center" style="cursor: pointer;">IDFB</div>
                                                    <div style="margin-top: 5px;"><input placeholder="Từ khóa ..." class="form-control form-control-sm" value="" style="min-width: 90px;" /></div>
                                                </th>
                                                <th colspan="1" role="columnheader">
...
$url="https://muasubngon.com/api/service/list-follow";
...
<?php
$result = mysqli_query($ketnoi,"SELECT * FROM `sub` WHERE `username` = '".$user['username']."'");
while($row = mysqli_fetch_assoc($result)){
    ...
    <td role="cell">
        <p><b class="text-danger"><?=$arr["amount"] * $ratesub?></b>&nbsp;<sup>VNĐ</sup></p>
    </td>
    <td role="cell">

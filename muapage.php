...
if( isset($_POST['submit']) && isset($_POST['idpage']) && isset($_POST['amount'])){
$amount=$_POST['amount'];
$idpage=$_POST['idpage'];
...
$url="https://muasubngon.com/api/service/order-like-page";
...
$ch = curl_init();
    ...
    curl_setopt( $ch, CURLOPT_POSTFIELDS, "idpage=".$idpage."&amount=".$amount."&note=".$note);
	$headers = array(
...
$create = $ketnoi->query("INSERT INTO `page` SET 
                                `order_code` = '".$a["data"]["order_code"]."',
                                `username` = '".$user["username"]."',
                                `amount` = '".$a["data"]["amount"]."',
                                `idpage` = '".$a["data"]["idpage"]."'");
                ...
                <div class="card-body">
                    <form action="" method="post">
                        ...
                        <div class="form-group row">
                            <label class="col-md-2">ID PAGE:</label>
                            <div class="col-md-10"><input type="text" class="form-control" name="idpage" placeholder="Nhập id page cần tăng" value="" /></div>
                        </div>
                        <div class="form-group row">
...
<ul class="pt-2"><li>Mua bằng ID Page.</li><li>Tất cả đều là người dùng Việt Nam.</li><li>Đơn càng to thì lên sẽ càng nhanh</li><li>Hiện tại hệ thống có thể tăng tối đa 1m like / 1 page</li><li>Sau khi mua thì chỉ từ 1p đến 20p bắt đầu chạy.</li><li>Chế độ bảo hành trong 10 ngày khi tụt trên 10% sẽ được buff lại số đã tụt.</li><li>Chúng tôi không hoàn tiền cho đơn đã thanh toán.</li></ul>
                                </div>
                            </div>
                        </div>
                        <div class="form-group row">
                            <div class="col-sm-12 text-center">
                                <div class="alert alert-dismissible alert-success">
                                    <h3 class="font-bold">
                                        Tổng thanh toán: <span class="bold green"><span id="total_payment" class="text-danger"><?=$ratepage * 100?></span> VNĐ</span>
                                    </h3>
                                </div>
                            </div>
                        </div>
                                    ...
                                    <table role="table" class="table table-sm table-bordered table-striped table-hover">
                                        <thead>
                                            <tr role="row">
                                                ...
                                                <th colspan="1" role="columnheader">
                                                    <div title="Toggle SortBy" class="text-center" style="cursor: pointer;">ID PAGE</div>
                                                    <div style="margin-top: 5px;"><input placeholder="Từ khóa ..." class="form-control form-control-sm" value="" style="min-width: 90px;" /></div>
                                                </th>
                                                <th colspan="1" role="columnheader">
...
$url="https://muasubngon.com/api/service/list-like-page";
...
<?php
$result = mysqli_query($ketnoi,"SELECT * FROM `page` WHERE `username` = '".$user['username']."'");
while($row = mysqli_fetch_assoc($result)){
    ...
    <td role="cell">
        <p><b class="text-danger"><?=$arr["amount"] * $ratepage?></b>&nbsp;<sup>VNĐ</sup></p>
    </td>
    <td role="cell">
